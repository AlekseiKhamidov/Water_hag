<?php
 ...
 require_once "../config.php";
	...
	function selectLeads($partnerId = 0, $start, $end) {
		$sql = $partnerId ? "SELECT * FROM `leads_info` where `source` LIKE '%$partnerId' AND `created_at` >= $start AND `created_at` <= $end order by `created_at` desc"
							: "SELECT * FROM `leads_info` order by `created_at` desc";
